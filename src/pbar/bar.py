...
class Sticky:
	...
	def __init__(self, barObj: PBar,
		...
		@full: If `True`, the bar will take all the horizontal space of the terminal.
		"""
		self._bar = barObj
		self._bheight = barObj.size[1]
		self._oldValues = barObj.size, barObj.position, barObj.formatset
		self._pos = position
		self._full = full
	...
	def update(self):
		...
		if self._pos == "top":
			print(Term.margin(top=scrollNum), end="")
			self._bar.position = (posX, 1)
		else:
			scrollNum += 1
			print(Term.margin(bottom=scrollNum), end="")
			self._bar.position = (posX, -1)

		if self._full:
			self._bar.formatset["right"] = self._bar.formatset["left"] = ""
			self._bar.size = (Term.size()[0] - 4, self._bheight)


	def __enter__(self):
		scrollNum = self._bheight + 2
		if self._pos == "top":
			print(Term.scroll(-scrollNum) + Term.moveVert(scrollNum), end="")
		else:
			scrollNum += 1
			print(Term.scroll(scrollNum) + Term.moveVert(-scrollNum), end="")
		self.update()
	...
	def __exit__(self, type, value, traceback):
		...
		self._bar.clear()
		self._bar.size, self._bar.position, self._bar.formatset = self._oldValues
		del self