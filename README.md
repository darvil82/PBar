...
When setting `titleComments` as `True`, we can use Python comments to tell the wrapper what value `PBar.title` will have for each call. For example:
```py
@taskWrapper(PBar(), locals(), titleComments=True)
def myTasks():
	getReq()	# Getting requests
	processReq()	# Processing requests
	prntText()	# Printing
	testMgr()	# Testing
```
